...
class GANModel:
    ...
    def forward(self):
        #skeleton real_A
        self.skel_real_A = skeleton(self.real_A).to(self.gpu_ids[0])
        self.encoded_skel_real_A =  self.netG(self.skel_real_A)

        # generate fake_B
        self.fake_B, self.encoded_real_A = self.netG(self.real_A, self.labels, self.encoded_skel_real_A)
        ...
        for name in ['G', 'D']:
            if isinstance(name, str):
                ...
                if self.gpu_ids and torch.cuda.is_available():
                    ...
                    torch.save(net.cuda(self.gpu_ids[0]).state_dict(), save_path)
                else:
                    torch.save(net.cpu().state_dict(), save_path)
...
def skeleton(img_in_batch, input_nc=3):

    img_list = []
    skeleton_img_list = []
    for i in range(len(img_in_batch)):
        img_list.append(img_in_batch[i])
        # grayscale
        gray_img = transforms.ToPILImage()(img_list[i]).convert('L')
       
        # Invert the image
        invert_img = ImageOps.invert(gray_img)

        # pil image to ndarray
        invert_img = np.array(invert_img)

        # binarize
        thresh = threshold_otsu(invert_img)
        binary = invert_img > thresh

        # perform skeletonization
        skeleton = skeletonize(binary, method='lee')
        skeleton = Image.fromarray(np.uint8(skeleton)).convert('L')
        ...
        skeleton_img = Image.fromarray(np.uint8(skeleton)).convert('RGB')

        # image to ndarray
        skeleton_img = np.array(skeleton_img)
        
        skeleton_img.resize((input_nc, img_list[i].shape[1], img_list[i].shape[2]))
        #print("skeleton_img shape: ", skeleton_img.shape)
        skeleton_img_list.append(skeleton_img)
        

    # ndarray to tensor
    skeleton_img_list = np.array(skeleton_img_list)
    skeleton_img_tensor = torch.Tensor(skeleton_img_list)

    return skeleton_img_tensor